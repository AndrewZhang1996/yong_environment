...
//		this.save(user);  //保存角色的时候已经添加过一次了
	...
	public void saveUser(SysUser user, String selectedRoles, String selectedDeparts) {
		if(oConvertUtils.isNotEmpty(selectedDeparts)) {
			...
			for (String deaprtId : arr) {
				SysDepart userDepart = sysDepartMapper.selectById(deaprtId);
				orgCodes.append(userDepart.getOrgCode());
			}
			if (orgCodes.length() > 0) {
				user.setOrgCode(orgCodes.toString());
			}
		}
		//step.1 保存用户
		this.save(user);
	...
	@Override
	public List<SysUser> getUsersByNames(List<String> usernames) {
		return userMapper.getUsersByNames(usernames);
	}

	@Override
	public List<SysUser> getUsersByStatusAndOrg(Integer status, String orgCode) {
		return userMapper.getUsersByStatusAndOrg(status, orgCode);
	}
